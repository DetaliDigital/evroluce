$(document).ready(function() {
	...
	//$('input[name="phone"]').add('input[name="tel"]').inputmask("+7 (999) 999-99-99");
	...
	//filter resultbox mechanics
	var result_filter_el;

	function filterResultBox(el_this) {
	  var f_result_offset = $(el_this).offset().top - $(el_this).closest('.filter_block_inner').offset().top;
	  $('.filter_result_box').addClass('active');
	  $('.filter_result_box').css('top',f_result_offset + "px");

	  if ($(el_this).hasClass('mse2_number_slider') == false) {
	    result_filter_el = el_this[0];
	  }

	  if ($(window).width() < 1200) {
	    if ($('.filter_block').hasClass('active') == false) {
	      $('.filter_result_box').removeClass('active');
	    }
	  }
	}

	$('.filter_block label.for_checkbox input[type="checkbox"]').change(function() {
	  filterResultBox($(this));
	});

	$('.mse2_number_slider').on( "slidechange", function( event, ui ) {
	   filterResultBox($(this));
	});


	$('.filter_result_box .show_filter_results_button').click(function(e) {
	  setTimeout(function() {
	    $('.filter_result_box').removeClass('active');
	    ...
	    /*
	    if ($(result_filter_el).hasClass('mse2_number_slider') == false) {
	      $(result_filter_el).closest('.has_filter_toggler').find('.filter_toggler').eq(0).click();
	    }
	    */
	  },2400);
	...
	//left_toggling_menu mechanics
	 $('.left_toggling_menu > ul > li').each(function() {
	   if ($(this).find('li.active').length) {$(this).addClass('active');}
	 });

	$('.left_toggling_menu > ul > li > ul').not('.left_toggling_menu > ul > li.active > ul').slideUp(0);

	$('.left_toggling_menu > ul > li').each(function() {
	 var ul_navs = $(this).children('ul');

	  if (ul_navs.length) {
	    $('<span class="nav_toggler">').appendTo($(this).children('a'));
	  }
	});

	$('.left_toggling_menu > ul > li').on("click",".nav_toggler",function(e) {
	  e.stopPropagation();
	  e.preventDefault();
	...
	$('.toggling_content').on("click",function(e) {
	   e.stopPropagation();
	});

	/*
	$('.left_toggling_menu > ul > li > a').click(function(e) {
	  e.stopPropagation();
	  e.preventDefault();
	  $(this).closest('li').toggleClass('active');
	  $(this).closest('li').children('ul').slideToggle();
	...
	*/
	...
	//card specs tab wrap into toggling blocks
	$('.card_tab_specs_block h3').each(function() {
	  var new_wrap = $('<div class="card_tab_specs_item has_filter_toggler"></div>');
	  $(new_wrap).appendTo($('.card_tab_specs_block'));
	  $(this).next('.specs').addClass('toggling_content');
	  $(this).next('.specs').appendTo($(new_wrap));
	  $(this).prependTo($(new_wrap));

	});

	$('.card_tab_specs_item.has_filter_toggler:nth-child(2)').addClass('active');
	//--
	...
	$('.filter_block, .card_tab_specs_block').on('click','.filter_toggler',function(e) {
	  e.stopPropagation();
	...
	$('.card_tab_specs_block').on("click", ".card_tab_specs_item h3, .card_tab_specs_item .filter_toggler", function(e) {
	  e.stopPropagation();
	  $(this).closest('.has_filter_toggler').toggleClass('active');
	  $(this).closest('.has_filter_toggler').find('.toggling_content').slideToggle();
	  $(this).toggleClass('active');
	});
	//--
	...
	$('.cart_order_block .quick_cart_order_button').click(function(e) {
		e.preventDefault();
		if ($('.content_left_column.full_order').hasClass('full_order')) {
		$('.content_left_column.order.full_order').hide().removeClass('full_order');
		$('.content_left_column.order_quick').show();
		$('.sticky-wrapper .ms2_link').hide();
		$('.sticky-wrapper .the_form_item_accept').hide();
		}
		else {
		$('.content_left_column.order').show().addClass('full_order');
		$('.content_left_column.order_quick').hide();
		$('.sticky-wrapper .ms2_link').show();
		$('.sticky-wrapper .the_form_item_accept').show();
		}
	});
	//--
	...
	$('.content_right_column a.one_click_ref').click(function(e) {
		e.preventDefault();
		$(this).closest('.content_right_column .content_right_column_in > div').removeClass('active');
		$('.content_right_column .one_click_form_block').add('.one_click_form_block_wrap').addClass('active');
	...
	*/
	...
	if ($(window).width() < 1200) {
		...
		if ($(".brands_catalog").length) {
			$('.brands_catalog').each(function() {
				$(this).slick({
				    slidesToShow: 6,
				    slidesToScroll: 1,
				    arrows: true,
...
$('div.count').click(function(e){
		...
		miniShop2.Callbacks.Order.submit.response.success = function () {
		    yaCounter27810060.reachGoal('PLACE_AN_ORDER_CLICK');
		  }
